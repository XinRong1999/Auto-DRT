...
function scoreColumn(column, rows, role) {
  ...
  if (role === "zReal") {
    if (/(zreal|zre|real|rez|zprime|z')/.test(header) || header === "re") score += 9;
    if (stats.finiteCount >= 3) score += 1;
...
function columnStats(index, rows) {
  ...
  for (let i = 1; i < values.length; i += 1) {
    if (values[i] > values[i - 1] || values[i] < values[i - 1]) monotonicSteps += 1;
  }
  ...
  return {
    ...
    monotonicFraction: finiteCount > 1 ? monotonicSteps / (finiteCount - 1) : 0,
